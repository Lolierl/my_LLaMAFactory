...
def weighted_cross_entropy(
    ...
    weights2: Optional[torch.Tensor] = None,
    ignore_index: int = -100,
    ...
    """
    Args:
        source: 预测logits，shape [batch_size * seq_len, vocab_size]
        target: 目标标签，shape [batch_size * seq_len]
        weights: 权重张量，shape同target
        num_items_in_batch: batch中的有效项目数
        ignore_index: 需要忽略的索引值
        topk: 如果指定，则仅考虑预测中概率最高的top-k的token
        topp: 如果指定，则仅考虑累计概率达到top-p的token
    Returns:
        加权后的损失值
    """
    ...
    if topk != None:
        ...
        target_in_topk = (topk_idx == target_expanded).any(dim=-1)

        ratio = target_in_topk.float().mean()
        print(f"target_in_{topk} true ratio = {ratio.item():.4f}")
        per_token_loss = per_token_loss * target_in_topk.float()

    if topp != None:
        with torch.cuda.amp.autocast():
            probs = torch.softmax(valid_source, dim=-1)
            topk_probs, topk_idx = torch.topk(probs, k=1024, dim=-1)

            cum_probs = torch.cumsum(topk_probs, dim=-1)

            topp_mask = cum_probs <= topp
            topp_mask[..., 0] = True  

            batch_idx = torch.arange(valid_target.size(0), device=valid_target.device)
            target_exp = valid_target[:, None]  # [batch, 1]
            match = topk_idx == target_exp  # [batch, top_k]
            match_any = match.any(dim=-1)  

            target_pos = torch.argmax(match.int(), dim=-1)  # [batch]

            target_in_topp = match_any & (topp_mask[batch_idx, target_pos])
            ratio = target_in_topp.float().mean()
            print(f"target_in_{topp} true ratio = {ratio.item():.4f}")
        per_token_loss = per_token_loss * target_in_topp.float()

    valid_losses = per_token_loss
    with torch.no_grad():
        #valid_weights = weights[valid_mask]
        #valid_weights2 = weights2[valid_mask] if weights2 is not None else None
        log_prob = -valid_losses
        #log_prob_bound = torch.log(torch.tensor(prob_bound, device=valid_losses.device, dtype=valid_losses.dtype))
        #clipped_log_prob = torch.maximum(log_prob, log_prob_bound)

        #reweight_diff = (valid_weights - clipped_log_prob) / temp
        #reweight_diff = torch.clamp(torch.exp(reweight_diff), min=clip_min, max=clip_max)

        #log_context_prob = torch.cumsum(log_prob, dim=0)
        #log_context_prob = log_context_prob / log_prob.size(0)
        #context_p_theta = torch.exp(log_context_prob)

        
        #context_avg_prob = torch.cumsum(p_theta, dim=0)
        #context_avg_prob = context_avg_prob / p_theta.size(0)
        
        #p_ref = torch.exp(valid_weights)
        p_theta = torch.exp(log_prob)
        if use_dft_loss:
            weight_diff = p_theta  ### dft
        else:
        ...
        #weight_diff = valid_weights #improvedict
        #ratio_ones = (weight_diff == 1).float().mean()
        #print(f"weight_diff=1 ratio = {ratio_ones.item():.4f}")
        #weight_diff = p_ref
        #weight_diff = torch.where(p_theta > 0.01,  1.0-p_theta, torch.tensor(0.0, device=p_theta.device, dtype=p_theta.dtype)) ### reverse
        #clipped_weight_diff = torch.clamp(weight_diff, min=clip_min, max=clip_max)
    #mean_val = clipped_weight_diff.mean().clamp(min=1e-8)
    #normalized_weight = clipped_weight_diff / mean_val
    weighted_losses = valid_losses * weight_diff
    #weighted_losses = valid_losses * torch.exp(valid_weights)
    total_loss = weighted_losses.sum()
    if torch.is_tensor(num_items_in_batch):
        num_items_in_batch = num_items_in_batch.to(total_loss.device)
    if num_items_in_batch is not None:
...
def weighted_loss_func(outputs, labels, weights, weights2 = None, num_items_in_batch=None, topk = None, topp = None, drop_prob_threshold = None, use_dft_loss = False):
    ...
    if weights is not None:
        weights = torch.nn.functional.pad(weights, (0, 1), value=-100)
        weights = weights[..., 1:].contiguous()
        weights = weights.view(-1)
        weights = weights.to(logits.device)

    if weights2 is not None:
        weights2 = torch.nn.functional.pad(weights2, (0, 1), value=-100)
        weights2 = weights2[..., 1:].contiguous()
        weights2 = weights2.view(-1)
        weights2 = weights2.to(logits.device)

    loss, logs = weighted_cross_entropy(
        logits, 
        shift_labels, 
        weights, 
        weights2, 
        ...
        topk=topk, 
        topp=topp, 
        drop_prob_threshold=drop_prob_threshold,
