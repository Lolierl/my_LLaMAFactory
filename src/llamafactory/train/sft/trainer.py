...
from ..trainer_utils import create_custom_optimizer, create_custom_scheduler, weighted_loss_func
import numpy as np
    ...
    def compute_loss(
        ...
        if labels is not None:
            if self.compute_loss_func is not None:
                ...
                #else:
                #    weights = None
                #if inputs.get("weights2") is not None:
                #    weights2 = inputs.pop("weights2")
                #else:
                #    weights2 = None
                weights = None
                weights2 = None
                ...
                #else:
                #    topk = None
                loss, logs = self.compute_loss_func(outputs, labels, weights, weights2, num_items_in_batch=num_items_in_batch, topk=drop_topk, drop_prob_threshold=drop_prob_threshold, use_dft_loss=use_dft_loss)
                self.log(logs)
                #sft_loss = outputs["loss"].detach()  if isinstance(outputs, dict) else outputs[0].detach() 
                #self.log({"sft_loss": sft_loss, "dft_loss": loss})
            else: 
