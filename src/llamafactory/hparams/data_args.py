...
@dataclass
class RLreweightingArguments:
    r"""Arguments pertaining to RL reweighting."""
    rl_model_name: Optional[str] = field(
        default=None,
        metadata={"help": "rl model to do token reweighting."},
    )
    max_clip_value: float = field(
        default=6,
        metadata={"help": "clip for reweighting."},
    )
    min_clip_value: float = field(
        default=0.1,
        metadata={"help": "clip for reweighting."},
    )
    rl_reweighting_temperature: float = field(
        default=1.0,
        metadata={"help": "temperature for reweighting."},
    )
    def to_dict(self) -> dict[str, Any]:
        return asdict(self)